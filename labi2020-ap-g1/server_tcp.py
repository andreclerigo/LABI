...
def run_sorting( proc ):
    def getList(dict):  #Returns a list with all ids connected to the server
        return list(dict.keys())   #Makes a list out of dictionary keys

    clients_list = getList(proc['ids'])
    print(clients_list)  #Debug

    #Randomize list
    random.shuffle(clients_list)  #Updates the list
    print(clients_list)   #Debug
    chosen_client = random.choice(clients_list)  #Chooses a random client
    print(chosen_client) #Debug
    print(proc['ids'][chosen_client]['endpoint']) #Debug
    socket_client = proc['ids'][chosen_client]['endpoint']
    send_dict(socket_client, clients_list)  #A fucnao send é a correta???
    #Como mandar a mensagem para o cliente da lista de clientes em ordem random??
    return 
...
def main():
    ...
    if len(sys.argv) != 2:
        sys.exit("ERROR: Invalid parameters <server port>")
